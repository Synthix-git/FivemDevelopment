...
RegisterCommand("group", function(source, args)
    ...
    -- Permissão: só Admin pode usar; CONSOLE sempre pode
    if not isConsole and not vRP.HasGroup(Passport, "Admin") then
        TriggerClientEvent("Notify", source, "Permissão", "Não tens permissões.", "vermelho", 5000)
        return
    end
    ...
    -- Proteção: Staff não pode dar grupo Admin (só CONSOLE pode)
    if not isConsole and string.lower(group) == "admin" then
        TriggerClientEvent("Notify", source, "Bloqueado", "Não podes atribuir o grupo <b>Admin</b>.", "vermelho", 6000)
        return
    end

    -- Carrega hierarquia (se existir) para obter cargo
    local Global = module("vrp", "config/Global")
    ...
    if isConsole then
        print(("[GROUP] %s."):format(notifyMsg:gsub("<b>", ""):gsub("</b>", "")))
    else
        TriggerClientEvent("Notify", source, "Sucesso", notifyMsg .. ".", "verde", 5000)
    end
    ...
    local msg = table.concat({
        ...
        ("🏷️ **Grupo Adicionado:** %s"):format(group),
...
RegisterCommand("warn", function(source, args)
    ...
    if not isConsole and not vRP.HasPermission(Passport,"Admin",4) then
        TriggerClientEvent("Notify", source, "Permissão", "Apenas <b>Admin nível 2</b> (Dono) pode usar.", "vermelho", 5000)
        return
    end
...
RegisterCommand("fixperms", function(source)
    if source == 0 then -- Apenas console pode rodar
        ...
        -- Passaporte 8 → Admin 1
        vRP.SetPermission(8, "Admin", 1)
        print("Permissão Admin nível 1 atribuída ao passaporte 8")
    end
end)
----------------------------------------------------------
	---- GOD SYN
...
RegisterCommand("godarea", function(source, args)
    ...
    TriggerClientEvent("Notify", source, "sucesso", "God aplicado a "..total.." jogador(es) num raio de "..range.." metros.", 5000)
end)
...
RegisterCommand("daritem", function(source, args)
    local Passport = vRP.Passport(source)
    if not Passport or not vRP.HasGroup(Passport, "Admin", 4) then return end
