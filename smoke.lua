...
CreateThread(function()
    ...
    CreateThread(function()
        while true do
            ...
            if DoesEntityExist(ped) then
                ...
                for _, ent in ipairs(objs) do
                    if DoesEntityExist(ent) and _modelHashes[GetEntityModel(ent)] then
                        ...
                        if not IsEntityAttachedToEntity(ent, ped) then
                            ...
                            if dist(myPos, c) <= SCAN_RADIUS then
                ...
                -- Redundância com GetClosestObjectOfType
                for hash,_ in pairs(_modelHashes) do
                    local obj = GetClosestObjectOfType(myPos.x, myPos.y, myPos.z, SCAN_RADIUS, hash, false, false, false)
                    if obj ~= 0 and DoesEntityExist(obj) then
                        if not IsEntityAttachedToEntity(obj, ped) then
                            EnsureEntityTracked(obj, now)
                        end
                    end
                end
            end
    ...
    CreateThread(function()
        while true do
            ...
            -- target de overlay/timecycle (intenso)
            if needOverlay then
                Overlay.target = 249
            else
                Overlay.target = 0
