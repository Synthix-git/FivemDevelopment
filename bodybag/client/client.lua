...
---------------------------------------------------------------------
-- USO DO ITEM (o teu inventário dispara estes eventos)
---------------------------------------------------------------------
...
AddEventHandler("bodybag:Use", function()
  ...
  if p and p ~= 0 then
    ...
    --  NÃO apagues o ped aqui; espera confirmação do servidor
    TriggerServerEvent("bodybag:Try", {
      type = "ped",
      pos = { x = pos.x, y = pos.y, z = pos.z },
      heading = hdg,
...
AddEventHandler("bodybag:HoldKneel", function(duration)
  ...
  -- fallback de limpeza (o inventário também chama vRPC.Destroy no fim)
  CreateThread(function()
    Wait(250)
